...
async def acp_permission_callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if len(parts) >= 4:
        ...
        session_id = getattr(target_session, "session_id", "unknown") if target_session else "unknown"
    ...
    logger.info(f"[TG_PERM_DONE] user={user_id} session={session_id} req_id={req_id} action={action} elapsed={(time.monotonic()-_t0):.3f}s")
