...
def on_background_session_finished(session: Any) -> None:
    ...
    if (
        ...
        and active_streamers[user_id].session.session_id == session.session_id
    ):
    ...
    reply_markup = InlineKeyboardMarkup(
        [
            [
                ...
                InlineKeyboardButton(
                    "📥 下載 Session Log", callback_data=f"sess:download:{session.session_id}"
                ),
            ]
        ]
    )
    ...
    async def _safe_send_exit_alert() -> None:
        try:
            await bot_app.bot.send_message(
                chat_id=user_id, text=alert_text, reply_markup=reply_markup, parse_mode="Markdown"
            )
...
@restricted
async def session_restart_callback_handler(
    ...
    if not info:
        await query.edit_message_text(
            "⚠️ *重啟 token 已過期或無效*，請發送 `/aoh_new` 重新建立 Session。",
            parse_mode="Markdown",
        )
        return
    ...
    if not is_path_allowed(working_dir):
        await query.edit_message_text(
            "⛔ *無法重新啟動*：工作目錄超出允許範圍。", parse_mode="Markdown"
        )
        return
