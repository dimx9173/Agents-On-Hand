...
def test_session_store_edge(tmp_path):
    ...
    assert loaded2 == []
    # roundtrip
    rec = SessionRecord(session_id="s1", user_id=1, agent_key="bash", agent_name="Bash", command="bash", working_dir="/tmp", created_at=time.time())
    store.save_state([rec], {1: "s1"})
    loaded3, active3 = store.load_state()
    assert loaded3[0].session_id == "s1"

def test_logging_sanitize():
    from agents_on_hand.logging_setup import _sanitize, _REDACT_LABEL
...
async def test_session_manager_prune(tmp_path):
    ...
    s = AgentSession(session_id="sess_off", user_id=99, agent_key="bash", agent_name="Bash", command="bash", working_dir=tmp_path)
    s.is_running = False
