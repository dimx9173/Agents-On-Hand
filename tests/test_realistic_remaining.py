...
async def test_stream_handler_tool_and_throttle():
    ...
    streamer.start()
    # tool request
    evt = DriverEvent(event_type=DriverEvent.TOOL_REQUEST, request_id="t1", tool_name="bash", tool_args="ls -la")
...
def test_security_edge_cases():
    ...
    with patch("agents_on_hand.config.ALLOWED_TELEGRAM_USER_IDS", set()), patch("agents_on_hand.config.DEV_ALLOW_ALL", False):
...
def test_ansi_and_logging_extra(tmp_path):
    from agents_on_hand.ansi_cleaner import strip_ansi_codes, clean_cli_output, format_telegram_code_block, format_hermes_style
