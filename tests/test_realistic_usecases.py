...
async def test_journey_new_session_via_directory_browser(tmp_path):
    ...
    from agents_on_hand.ui.directory_browser import send_directory_browser, show_agent_selector, agent_start_callback_handler
    # browse
    update = MagicMock()
    ...
    with patch("agents_on_hand.ui.directory_browser.get_installed_cli_agents", return_value={}), patch("agents_on_hand.ui.directory_browser.get_path_token", return_value="p_0"):
...
async def test_journey_chat_send_and_offline_recovery(tmp_path):
    ...
    with patch("agents_on_hand.handlers.chat.session_manager") as sm, patch("agents_on_hand.handlers.chat.create_streamer_for_session") as mk_streamer, patch("agents_on_hand.handlers.chat.register_restart_info", return_value="r_abc12345"), patch("agents_on_hand.security.is_user_allowed", return_value=True):
        ...
        with patch("agents_on_hand.handlers.chat.active_streamers", {}) as active:
            update = MagicMock()
            update.effective_user = MagicMock(id=1)
            update.message = MagicMock()
...
async def test_journey_session_switch_and_logs():
    ...
    with patch("agents_on_hand.ui.session_menu.session_manager") as sm, patch("agents_on_hand.ui.session_menu.active_streamers", {}) as active, patch("agents_on_hand.security.is_user_allowed", return_value=True):
        sm.get_session.return_value = mock_sess
        ...
        ctx.bot = MagicMock()
        ctx.bot.send_message = AsyncMock()
        with patch("agents_on_hand.ui.session_menu.create_streamer_for_session") as mk_streamer, patch("agents_on_hand.ui.session_menu.format_telegram_code_block", return_value="```logs```"):
            mk_streamer.return_value = MagicMock()
            mk_streamer.return_value.start = MagicMock()
...
async def test_journey_background_finish_and_restart(tmp_path):
    ...
    from agents_on_hand.handlers.restart import on_background_session_finished, session_restart_callback_handler
    from agents_on_hand.session_manager import AgentSession
    # background finish
    sess = AgentSession(session_id="sess_bg", user_id=1, agent_key="bash", agent_name="Bash", command="bash", working_dir=tmp_path)
    sess.is_running = False
    with patch("agents_on_hand.handlers.restart.bot_app") as bot_app, patch("agents_on_hand.handlers.restart.register_restart_info", return_value="r_test1234"), patch("agents_on_hand.handlers.restart.active_streamers", {}):
