...
async def test_session_menu_full():
    ...
    with (
        ...
        with (
            ...
            patch(
                "agents_on_hand.ui.session_menu.format_telegram_code_block", return_value="```x```"
            ),
            patch("agents_on_hand.ui.session_menu.bot_app", MagicMock()),
        ):
...
def test_bot_facade_imports():
    from agents_on_hand.bot import help_command, main, new_command
